...
- コンテストによっては、複数のプログラム間で通信する形式の問題（「コミュニケーション形式」や「インタラクティブ形式」と呼ばれる）が出題される
	...
	- **コミュニケーション形式の問題では、出力完了時に `std::flush` または `std::endl` を使って、必ずフラッシュする**よう心がける
- コミュニケーション形式の問題の例:
...
### 2.3 入力の区切り
- 標準入力で複数の数を与える場合、数どうしは空白文字（半角スペース、タブ、改行）で区切る
...
- ①, ② の方法で標準入出力を高速化すると、次のようなリスクが発生する
	- ① を適用すると、`std::cin` が `std::cout` のフラッシュを実行しなくなるため、コミュニケーション形式の問題でプログラムの途中の出力が送信されなくなる
		- 出力の完了時に明示的にフラッシュして対処する（「1.6 コミュニケーション形式の問題におけるフラッシュ」参照）
...
### 4.3 bool 型の値を文字列で出力する | `std::boolalpha`
- 読み方: ブールアルファ
- `<iostream>` ヘッダに含まれる
...
- `std::clog` はバッファリングあり、`std::cerr` はバッファリング無し
