...
- 競技プログラミングでは、フラッシュが行われないと出力がジャッジされないが、終了時の自動フラッシュがあるため、**通常の問題では明示的にフラッシュする必要はない**
	- 逆に、**頻繁にフラッシュをすると実行時間が遅くなる**ため、改行は `std::endl`（改行 + フラッシュ）ではなく、改行文字 `'\n'` を使う
...
- これらの設定を、プログラムで標準入出力を行う前に 1 回だけ行うことで、標準入出力の処理時間を短縮できる
	- この設定を適用すると、C++ の標準入出力と C 言語の入出力関数が混在するプログラムを書いた際、入出力の順序に不整合が生じる可能性があるため、注意が必要
...
### 6.1 リダイレクトとは
- 通常、プログラムの標準入力はキーボード、標準出力は画面（コンソール）に紐づいている
- リダイレクトは、プログラムの標準入出力先を変更（切り替え）する機能
  - 例：ファイルからデータを読み込みたいときに標準入力をファイルに切り替える  
  - 例：プログラムの出力をファイルに保存したいときに標準出力をファイルに切り替える
- 競技プログラミングでは、ローカルでテストするときにリダイレクトを使い、「テスト用ファイルから入力して、結果をファイルへ書き出す」ケースがある

### 6.2 リダイレクトの例

#### 入力をファイルから読み込む
- コマンドライン上で、実行ファイルの後ろに `< 入力ファイル` と書くことで、標準入力がそのファイルに差し替わる 

```bash
./a.out < input.txt
```

- これによって、`std::cin` からの入力はすべて `input.txt` の内容から読み込まれる

#### 出力をファイルに書き込む
- コマンドライン上で、`> 出力ファイル` と書くことで、標準出力がファイルに差し替わる  
...
#### 入力と出力を同時にリダイレクトする
- 入出力の両方をファイルに切り替えることができる

```bash
./a.out < input.txt > output.txt
```

- これで `std::cin` は `input.txt`、`std::cout` は `output.txt` を使うようになる
...
### 6.4 まとめ
- リダイレクトを使うことで、コンソールへの手入力・画面への出力を省き、ファイルでデータをやり取りできます。  
- 標準入出力を使う C++ コードを書いてさえいれば、プログラム側に特別な変更を加えることなくリダイレクトが利用可能です。
