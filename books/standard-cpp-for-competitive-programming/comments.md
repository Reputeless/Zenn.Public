...
### 1.3 単一行コメントの特殊な仕様
- 単一行コメントの行末がバックスラッシュ「`\`」で終わる場合、次の行もコメントとして扱われる特殊な仕様があるため、注意が必要
	- C++ の「プリプロセッサ」という機能の仕様（行末に `\` があると次の行も同じ行として扱われる）が原因で、コメント範囲が継続する
...
## 5. ドキュメントコメントを知る
- ドキュメントコメントとは、関数やクラスの説明を記述するためのコメントのこと
- 他人に公開する自作ライブラリを作る場合は、ドキュメントコメントを書くことで、他の人が使い方を理解しやすくなる
