...
- 単一行コメントの行末がバックスラッシュ「`\`」で終わる場合、次の行もコメントとして扱われるため注意が必要
	- C++ のプリプロセッサ機能の「行末に `\` があれば次の行も同じ行として扱う」という仕様の影響でコメント範囲が継続するのが原因
	- あくまでプリプロセッサという別の機能の仕様の副作用。コメント内で意図的に使用するのは避けるべき
...
- 学習が進めば、同じコードでも、次のようにコメントを省略しても理解できるようになる
...
- ただし、あまり知られていないテクニックであり、読む人によっては混乱を招くため、普段のコーディングでは使わないほうがよい
