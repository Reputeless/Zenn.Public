...
### 対応している音声フォーマット
OpenSiv3D v0.6.3 では、次のような音声フォーマットの読み込みがサポートされています。一部の音声フォーマットは OS によって対応状況が異なります。
...
OpenSiv3D v0.6.3 Web 版では、ピッチシフトフィルタを利用できません。`GlobalAudio::SupportsPitchShift()` を使うと、現在の実行環境でピッチシフトフィルタを利用できるかを取得できます。
