...
Windows 版では、アプリケーションの実行中に Alt + Enter キーを押すことで**全画面モード**にできます。挙動としてはほぼフルスクリーンモードですが、シーンのリサイズモードが `ResizeMode::Keep` に設定され、シーンのサイズが変化しません。フルスクリーンモードや解像度の変更に対応していないアプリケーションを、全画面で大きく表示して実行したいときに役立ちます。
